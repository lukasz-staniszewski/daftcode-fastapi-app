...
def test_register():
    ...
    response = client.post("/register/", json={"name": "Lukasz", "surname": "Staniszewski"})
    assert response.status_code == 201
    date_then = date_today + timedelta(days=18)
    assert response.json() == {"id": 1, "name": "Lukasz", "surname": "Staniszewski", "register_date": f"{date_today}", "vaccination_date": f"{date_then}"}

    response = client.post("/register/", json={"name": "Marcin", "surname": "Najman"})
    assert response.status_code == 201
    date_then = date_today + timedelta(days=12)
    assert response.json() == {"id": 2, "name": "Marcin", "surname": "Najman", "register_date": f"{date_today}", "vaccination_date": f"{date_then}"}

    response = client.post("/register/", json={"name": "Krzysztof", "surname": "Kolumb"})
    assert response.status_code == 201
    date_then = date_today + timedelta(days=15)
    assert response.json() == {"id": 3, "name": "Krzysztof", "surname": "Kolumb", "register_date": f"{date_today}", "vaccination_date": f"{date_then}"}

    response = client.post("/register/", json={"name": "Krzysztof", "surname": "Kolumb"})
    assert response.status_code == 201
    date_then = date_today + timedelta(days=15)
    assert response.json() != {"id": 5, "name": "Krzysztof", "surname": "Kolumb", "register_date": f"{date_today}", "vaccination_date": f"{date_then}"}


def test_patient():
    date_today = date.today()
    response_post = client.post("/register/", json={"name": "Lukasz", "surname": "Staniszewski"})
    assert response_post.status_code == 201
    date_then = date_today + timedelta(days=18)

    response_get = client.get("/patient/1")
    assert response_get.status_code == 200
    assert response_get.json() == {"id": 1, "name": "Lukasz", "surname": "Staniszewski", "register_date": f"{date_today}", "vaccination_date": f"{date_then}"}
